...
table#calendar td#<%=LCase(Left(TodayMonthName,3))%><%=TodayDateNumber%><%=TodayYearNumber%> {border: 1px solid black;}
table#calendar td#<%=LCase(Left(CurrentDateMonthName,3))%><%=CurrentDateNumber%><%=CurrentDateYearNumber%> {background-color: yellow; border: 1px solid black;}
